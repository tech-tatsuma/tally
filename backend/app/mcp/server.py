...
async def list_accounts():
    async with SessionLocal() as s: return clean(await FinanceService(s, USER_ID).list_accounts())


@mcp.tool(description="Get one account and its current calculated balance by exact account ID.")
async def get_account(account_id: str):
    async with SessionLocal() as s: return clean(await FinanceService(s, USER_ID).get_account(uuid.UUID(account_id)))
...
async def create_account(name: str, account_type: str = "bank", initial_balance: str = "0", institution_name: str | None = None):
    async with SessionLocal() as s: return clean(await FinanceService(s, USER_ID).create_account(AccountCreate(name=name, account_type=account_type, initial_balance=Decimal(initial_balance), institution_name=institution_name)))
...
async def update_account(account_id: str, name: str | None = None, description: str | None = None):
    async with SessionLocal() as s: return clean(await FinanceService(s, USER_ID).update_account(uuid.UUID(account_id), AccountUpdate(name=name, description=description)))


@mcp.tool(description="Archive an account by exact ID. Its history remains intact.")
async def delete_account(account_id: str):
    async with SessionLocal() as s: await FinanceService(s, USER_ID).archive_account(uuid.UUID(account_id)); return {"archived": account_id}
...
async def create_transaction(account_id: str, transaction_type: str, amount: str, occurred_at: str, title: str, category_id: str | None = None, destination_account_id: str | None = None, description: str | None = None, journal: str | None = None):
    ...
    async with SessionLocal() as s: return clean((await FinanceService(s, USER_ID).create_transaction(data)).__dict__)
...
async def update_transaction(transaction_id: str, amount: str | None = None, title: str | None = None, description: str | None = None, journal: str | None = None):
    async with SessionLocal() as s: return clean((await FinanceService(s, USER_ID).update_transaction(uuid.UUID(transaction_id), TransactionUpdate(amount=Decimal(amount) if amount else None, title=title, description=description, journal=journal))).__dict__)


@mcp.tool(description="Delete one transaction by exact ID. Search and confirm candidates before calling this tool.")
async def delete_transaction(transaction_id: str):
    async with SessionLocal() as s: await FinanceService(s, USER_ID).delete_transaction(uuid.UUID(transaction_id)); return {"deleted": transaction_id}
...
async def list_recurring_transactions():
    ...
    async with SessionLocal() as s: return clean([r.__dict__ for r in await s.scalars(select(RecurringTransaction).where(RecurringTransaction.user_id == USER_ID))])
...
async def update_recurring_transaction(recurring_id: str, amount: str | None = None, title: str | None = None):
    async with SessionLocal() as s: return clean((await FinanceService(s, USER_ID).update_recurring(uuid.UUID(recurring_id), RecurringUpdate(amount=Decimal(amount) if amount else None, title=title))).__dict__)
...
async def get_current_assets():
    async with SessionLocal() as s: return clean(await FinanceService(s, USER_ID).dashboard())
...
async def get_asset_history(start: str, end: str, interval: str = "monthly"):
    async with SessionLocal() as s: return clean(await AnalyticsService(s, USER_ID).asset_history(date.fromisoformat(start), date.fromisoformat(end), interval))
...
async def get_cashflow_summary(start: str, end: str, interval: str = "monthly"):
    async with SessionLocal() as s: return clean(await AnalyticsService(s, USER_ID).cashflow(date.fromisoformat(start), date.fromisoformat(end), interval))
...
async def get_category_spending(start: str, end: str):
    async with SessionLocal() as s: return clean(await AnalyticsService(s, USER_ID).category_spending(date.fromisoformat(start), date.fromisoformat(end)))
