...
3. 「口座」から銀行口座、現金、投資、クレジットカードなどを追加します。新規ユーザーには「現金」口座が自動で作成されます。
...
すべての家計データはAPI側でユーザーIDを条件にして取得・更新されます。MCPもアクセストークンからユーザーを確定するため、他ユーザーの情報が混在することはありません。
...
1. アプリの「設定」→「MCP連携」からユーザー専用トークンを作成します。
2. トークンは作成直後に一度だけ表示されるため、安全な場所へ保管します。
3. stdioで起動する場合は、次のようにトークンを環境変数として渡します。

```bash
cd backend
MCP_ACCESS_TOKEN=<設定画面で発行したトークン> uv run python -m app.mcp.server
```

HTTP接続時は、`Authorization: Bearer <token>` ヘッダーを使用してください。トークンは設定画面からいつでも無効化できます。
