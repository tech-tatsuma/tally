...
type Recurring = { id: string; account_id: string; category_id?: string; type: "income" | "expense"; amount: number; title: string; frequency: string; execution_day: number; next_execution_date: string; enabled: boolean };
...
const demoRecurring: Recurring[] = [
  { id: "r1", account_id: "a1", category_id: "c3", type: "expense", amount: 80000, title: "家賃", frequency: "monthly", execution_day: 25, next_execution_date: "2026-08-25", enabled: true },
  { id: "r2", account_id: "a1", category_id: "c5", type: "expense", amount: 1490, title: "Netflix", frequency: "monthly", execution_day: 15, next_execution_date: "2026-09-15", enabled: true },
  { id: "r3", account_id: "a1", category_id: "c6", type: "income", amount: 350000, title: "給与", frequency: "monthly", execution_day: 25, next_execution_date: "2026-08-25", enabled: true },
];
...
export function WalletApp() {
  ...
  useEffect(() => {
    Promise.all([fetch(`${API}/accounts`), fetch(`${API}/categories`), fetch(`${API}/transactions?page_size=100`), fetch(`${API}/recurring-transactions`)]).then(async ([a, c, t, r]) => {
      if (![a, c, t, r].every((x) => x.ok)) throw new Error();
      setAccounts(await a.json()); setCategories(await c.json()); setTransactions((await t.json()).items); setRecurring(await r.json()); setConnected(true);
...
function AccountDetail({ id, accounts, categories, transactions, go }: PageProps & { id: string }) {
  ...
  return <div className="page"><button className="back-link" onClick={() => go("/accounts")}>← 口座一覧へ</button><PageHeader eyebrow={account.institution_name || account.account_type} title={account.name} action={<button className="secondary">口座を編集</button>} /><section className="account-detail-hero"><span>現在残高</span><strong>{money(account.current_balance)}</strong><small>取引履歴から自動計算</small></section><div className="dashboard-grid"><section className="panel span-2"><SectionHead title="残高の推移" /><AssetChart /></section><section className="panel span-2"><SectionHead title="この口座の取引" /><TransactionList items={items} accounts={accounts} categories={categories} /></section></div></div>;
}

function RecurringPage({ recurring, accounts, categories, setRecurring, notify }: PageProps) {
  const toggle = (item: Recurring) => { setRecurring((all) => all.map((x) => x.id === item.id ? { ...x, enabled: !x.enabled } : x)); notify(item.enabled ? "定期取引を停止しました" : "定期取引を再開しました"); };
  return <div className="page"><PageHeader eyebrow="AUTOMATION" title="固定費・定期収入" action={<button className="primary">＋ 定期取引を追加</button>} /><div className="info-banner"><span>↻</span><p><strong>毎月の記録を自動化</strong>　設定した日に取引が自動で作成されます。二重登録はされません。</p></div><section className="panel recurring-list"><div className="table-caption"><span>{recurring.length}件の定期取引</span><span>次回実行日順</span></div>{recurring.map((r) => <article key={r.id} className={!r.enabled ? "disabled" : ""}><span className="recurring-icon">{r.type === "income" ? "+" : "↻"}</span><div><h2>{r.title}</h2><p>{categories.find((c) => c.id === r.category_id)?.name} · {accounts.find((a) => a.id === r.account_id)?.name}</p></div><strong className={r.type === "income" ? "amount-in" : ""}>{r.type === "income" ? "+" : "−"}{money(r.amount)}</strong><div className="schedule"><b>毎月 {r.execution_day}日</b><small>次回 {r.next_execution_date.replaceAll("-", "/")}</small></div><label className="switch"><input type="checkbox" checked={r.enabled} onChange={() => toggle(r)} /><span /></label><button className="more" aria-label={`${r.title}のメニュー`}>•••</button></article>)}</section></div>;
}

function Analytics({ transactions, categories }: PageProps) {
  const [period, setPeriod] = useState("monthly");
  const bars = [45, 56, 49, 64, 58, 72]; const expenses = [62, 54, 68, 51, 66, 48];
  const categoryTotals = categories.filter((c) => c.type === "expense").map((c) => ({ ...c, amount: transactions.filter((t) => t.category_id === c.id).reduce((n, t) => n + Number(t.amount), 0) })).sort((a, b) => b.amount - a.amount);
  return <div className="page"><PageHeader eyebrow="INSIGHTS" title="収支分析" /><div className="analysis-controls"><div className="segmented"><button className={period === "daily" ? "active" : ""} onClick={() => setPeriod("daily")}>日次</button><button className={period === "monthly" ? "active" : ""} onClick={() => setPeriod("monthly")}>月次</button><button className={period === "yearly" ? "active" : ""} onClick={() => setPeriod("yearly")}>年次</button></div><button className="secondary">2026年 ▾</button></div><section className="metric-grid"><Metric label="収入" value={money(2_120_000)} note="前年同期比 +4.8%" tone="income"/><Metric label="支出" value={money(1_186_400)} note="前年同期比 −2.1%" tone="expense"/><Metric label="純収支" value={money(933_600)} note="貯蓄率 44%" tone="balance"/></section><div className="analytics-grid"><section className="panel span-2"><SectionHead title="収入と支出" /><div className="bar-chart">{bars.map((h, i) => <div key={i}><span><i className="income-bar" style={{ height: `${h}%` }}/><i className="expense-bar" style={{ height: `${expenses[i]}%` }}/></span><small>{i + 3}月</small></div>)}</div><div className="legend"><span><i className="income-color"/>収入</span><span><i className="expense-color"/>支出</span></div></section><section className="panel category-analysis"><SectionHead title="カテゴリ別支出" /><div className="donut" aria-label="カテゴリ別支出の円グラフ"><strong>合計<em>{money(categoryTotals.reduce((n, c) => n + c.amount, 0))}</em></strong></div>{categoryTotals.slice(0, 5).map((c) => <div className="category-row" key={c.id}><i style={{ background: c.color }}/><span>{c.name}</span><strong>{money(c.amount)}</strong></div>)}</section></div></div>;
}
