...
async function fetchAllTransactions(): Promise<Transaction[]> {
  ...
  do { const response = await apiFetch(`${API}/transactions?page=${page}&page_size=100`); if (!response.ok) throw new Error("transactions"); const payload = await response.json(); items.push(...payload.items); total = payload.total; page += 1; } while (items.length < total);
  return items;
}
...
function AuthScreen({ notice, authenticated }: { notice: string; authenticated: (user: User) => void }) {
  const initialMode = typeof location !== "undefined" && location.pathname === "/reset-password" ? "reset" : "login";
  const [mode, setMode] = useState<"login" | "register" | "forgot" | "reset">(initialMode);
  const [message, setMessage] = useState(notice); const [busy, setBusy] = useState(false);
  const [resetToken, setResetToken] = useState(() => typeof location !== "undefined" ? new URLSearchParams(location.search).get("token") || "" : "");
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setBusy(true); setMessage(""); const fd = new FormData(event.currentTarget);
    try {
      if (mode === "forgot") {
        const response = await apiFetch(`${API}/auth/forgot-password`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: fd.get("email") }) });
        if (!response.ok) throw new Error(); const payload = await response.json();
        if (payload.debug_reset_token) { setResetToken(payload.debug_reset_token); setMode("reset"); setMessage("開発環境用の再設定画面を開きました。"); }
        else setMessage("登録済みの場合、再設定メールを送信しました。メールをご確認ください。");
        return;
      }
      if (mode === "reset") {
        const password = String(fd.get("password")); if (password !== fd.get("confirm_password")) throw new Error("パスワードが一致しません");
        const response = await apiFetch(`${API}/auth/reset-password`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token: resetToken, new_password: password }) });
        if (!response.ok) throw new Error("リンクの期限が切れているか、パスワードの条件を満たしていません");
        history.replaceState({}, "", "/"); setMode("login"); setMessage("パスワードを更新しました。新しいパスワードでログインしてください。"); return;
      }
      const endpoint = mode === "register" ? "register" : "login";
      const payload = mode === "register" ? { name: fd.get("name"), email: fd.get("email"), password: fd.get("password") } : { email: fd.get("email"), password: fd.get("password"), remember_me: fd.get("remember_me") === "on" };
      const response = await apiFetch(`${API}/auth/${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!response.ok) { const error = await response.json().catch(() => ({})); throw new Error(error.detail || "入力内容を確認してください"); }
...
export function WalletApp() {
  ...
  const loadFinanceData = async () => {
    const [a, c, t, r] = await Promise.all([apiFetch(`${API}/accounts`), apiFetch(`${API}/categories`), fetchAllTransactions(), apiFetch(`${API}/recurring-transactions`)]);
  ...
  useEffect(() => {
    apiFetch(`${API}/auth/me`).then(async (response) => {
      ...
      const current = await response.json(); setUser(current); setAuthState("authenticated");
      await loadFinanceData();
    }).catch(() => { setConnected(false); setAuthNotice("サーバーに接続できません。立ち上げ方法を確認してから再試行してください。"); setAuthState("anonymous"); });
  }, []);

  if (authState === "loading") return <div className="auth-shell"><div className="auth-card loading-card"><span className="brand-mark">t</span><p>安全なセッションを確認しています…</p></div></div>;
  if (authState === "anonymous" || !user) return <AuthScreen notice={authNotice} authenticated={(value) => { setUser(value); setAuthState("authenticated"); setAuthNotice(""); loadFinanceData().catch(() => setConnected(false)); go("/dashboard"); }} />;

  const logout = async () => { await apiFetch(`${API}/auth/logout`, { method: "POST" }).catch(() => undefined); setUser(null); setAuthState("anonymous"); setAccounts([]); setCategories([]); setTransactions([]); setRecurring([]); setConnected(false); go("/"); };
  ...
  else if (path === "/settings") content = <Settings notify={notify} user={user} setUser={setUser} />;
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot"><button className="profile-shortcut" onClick={() => go("/settings")}><span className="avatar">{user.name[0]}</span><span><strong>{user.name}</strong><small>{user.role === "admin" ? "管理者" : "一般ユーザー"}</small></span></button><button className="logout-button" onClick={logout}>ログアウト</button></div>
    </aside>
    <main className="main"><header className="mobile-header"><button className="brand" onClick={() => go("/dashboard")}><span className="brand-mark">t</span><span>tally</span></button><span>{active[2]}</span><button className="mobile-profile" aria-label="設定を開く" onClick={() => go("/settings")}>{user.name[0]}</button></header>{content}</main>
...
function Transactions({ accounts, categories, transactions, setTransactions, go, notify, connected }: PageProps) {
  ...
  const remove = async (t: Transaction) => { if (!confirm(`「${t.title}」を削除しますか？`)) return; if (connected) await apiFetch(`${API}/transactions/${t.id}`, { method: "DELETE" }); setTransactions((all) => all.filter((x) => x.id !== t.id)); setSelected(null); notify("取引を削除しました"); };
...
function TransactionForm({ accounts, categories, setTransactions, go, notify, connected }: PageProps) {
  ...
  return <div className="page form-page"><button className="back-link" onClick={() => go("/transactions")}>← 取引一覧へ</button><PageHeader eyebrow="NEW TRANSACTION" title="取引を記録する" /><form className="transaction-form panel" onSubmit={submit}><div className="type-switch"><button type="button" className={kind === "expense" ? "active" : ""} onClick={() => setKind("expense")}>支出</button><button type="button" className={kind === "income" ? "active" : ""} onClick={() => setKind("income")}>収入</button><button type="button" className={kind === "transfer" ? "active" : ""} onClick={() => setKind("transfer")}>振替</button></div>
    <label className="amount-field"><span>金額</span><div><b>¥</b><input required name="amount" type="number" inputMode="numeric" min="1" step="1" placeholder="0" /></div></label>
...
function RecurringPage({ recurring, accounts, categories, setRecurring, notify, connected }: PageProps) {
  ...
  const toggle = async (item: Recurring) => { const enabled = !item.enabled; try { if (connected) { const response = await apiFetch(`${API}/recurring-transactions/${item.id}/${enabled ? "enable" : "disable"}`, { method: "POST" }); if (!response.ok) throw new Error(); } setRecurring((all) => all.map((x) => x.id === item.id ? { ...x, enabled } : x)); notify(enabled ? "定期取引を再開しました" : "定期取引を停止しました"); } catch { notify("状態を変更できませんでした"); } };
  const remove = async (item: Recurring) => { setMenu(null); if (!confirm(`「${item.title}」の定期取引を削除しますか？`)) return; try { if (connected) { const response = await apiFetch(`${API}/recurring-transactions/${item.id}`, { method: "DELETE" }); if (!response.ok) throw new Error(); } setRecurring((all) => all.filter((x) => x.id !== item.id)); notify("定期取引を削除しました"); } catch { notify("定期取引を削除できませんでした"); } };
...
function Settings({ notify, user, setUser }: { notify: (message: string) => void; user: User; setUser: (user: User) => void }) {
  const fileRef = useRef<HTMLInputElement>(null); const [restoring, setRestoring] = useState(false); const [editing, setEditing] = useState(false); const [passwordOpen, setPasswordOpen] = useState(false);
  const [tokens, setTokens] = useState<ApiToken[]>([]); const [revealedToken, setRevealedToken] = useState(""); const [users, setUsers] = useState<User[]>([]);
  useEffect(() => { apiFetch(`${API}/auth/api-tokens`).then((r) => r.ok ? r.json() : []).then(setTokens); if (user.role === "admin") apiFetch(`${API}/admin/users`).then((r) => r.ok ? r.json() : []).then(setUsers); }, [user.role]);
  const saveProfile = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const fd = new FormData(event.currentTarget); try { const response = await apiFetch(`${API}/auth/me`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: fd.get("name"), timezone: fd.get("timezone"), currency: fd.get("currency") }) }); if (!response.ok) throw new Error(); setUser(await response.json()); setEditing(false); notify("プロフィールを更新しました"); } catch { notify("プロフィールを更新できませんでした"); } };
  const changePassword = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const fd = new FormData(event.currentTarget); if (fd.get("new_password") !== fd.get("confirm_password")) { notify("新しいパスワードが一致しません"); return; } const response = await apiFetch(`${API}/auth/change-password`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ current_password: fd.get("current_password"), new_password: fd.get("new_password") }) }); if (response.ok) { setPasswordOpen(false); notify("パスワードを変更しました"); } else notify("現在のパスワードまたは入力条件を確認してください"); };
  const backup = async () => { try { const r = await apiFetch(`${API}/backup`); if (!r.ok) throw new Error(); const data = await r.json(); const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `tally-backup-${new Date().toISOString().slice(0, 10)}.json`; link.click(); URL.revokeObjectURL(link.href); notify("バックアップを書き出しました"); } catch { notify("バックアップに失敗しました"); } };
  const restore = async (file?: File) => { if (!file) return; if (!confirm("現在のデータをバックアップ内容で置き換えます。続けますか？")) return; setRestoring(true); try { const payload = JSON.parse(await file.text()); if (payload.schema_version !== 1 || !payload.data) throw new Error(); const r = await apiFetch(`${API}/backup/restore?confirm=true`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); if (!r.ok) throw new Error(); notify("バックアップを復元しました。画面を更新します"); setTimeout(() => location.reload(), 800); } catch { notify("このバックアップは復元できませんでした"); } finally { setRestoring(false); if (fileRef.current) fileRef.current.value = ""; } };
  const deleteAll = async () => { if (!confirm("すべての口座・カテゴリ・取引・定期取引を完全に削除します。元に戻せません。続けますか？")) return; const response = await apiFetch(`${API}/admin/data?confirm=true`, { method: "DELETE" }); if (response.ok) { notify("すべての家計データを削除しました"); setTimeout(() => location.reload(), 700); } else notify("データを削除できませんでした"); };
  const createToken = async () => { const response = await apiFetch(`${API}/auth/api-tokens`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "MCP連携", expires_in_days: 365 }) }); if (!response.ok) { notify("MCPトークンを作成できませんでした"); return; } const created = await response.json(); setTokens((all) => [created, ...all]); setRevealedToken(created.token); };
  const revokeToken = async (item: ApiToken) => { if (!confirm(`「${item.name}」を無効にしますか？`)) return; const response = await apiFetch(`${API}/auth/api-tokens/${item.id}`, { method: "DELETE" }); if (response.ok) { setTokens((all) => all.filter((x) => x.id !== item.id)); notify("MCPトークンを無効にしました"); } };
  const updateManagedUser = async (item: User, patch: Partial<Pick<User, "role" | "is_active">>) => { const response = await apiFetch(`${API}/admin/users/${item.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) }); if (response.ok) { const updated = await response.json(); setUsers((all) => all.map((x) => x.id === updated.id ? updated : x)); notify("ユーザー権限を更新しました"); } else notify("ユーザー権限を更新できませんでした"); };
  return <div className="page settings-page"><PageHeader eyebrow="PREFERENCES" title="設定" /><section className="settings-section panel"><div><h2>プロフィール</h2><p>表示名と地域設定を編集できます。メールアドレスはログインIDとして保護されています。</p></div><div className="profile-row"><div className="avatar large-avatar">{user.name[0]}</div><div><strong>{user.name}</strong><span>{user.email}</span><small className="role-badge">{user.role === "admin" ? "管理者" : "一般ユーザー"}</small></div><div className="profile-actions"><button className="secondary" onClick={() => setEditing(true)}>プロフィールを編集</button><button className="secondary" onClick={() => setPasswordOpen(true)}>パスワード変更</button></div></div></section><section className="settings-section panel"><div><h2>MCP連携</h2><p>このユーザー専用のトークンです。MCPから他ユーザーの情報へアクセスすることはできません。</p></div><div className="token-toolbar"><button className="secondary" onClick={createToken}>新しいトークンを作成</button></div>{revealedToken && <div className="token-reveal"><strong>このトークンは今だけ表示されます</strong><code>{revealedToken}</code><button className="secondary" onClick={() => navigator.clipboard.writeText(revealedToken).then(() => notify("トークンをコピーしました"))}>コピー</button></div>}<div className="token-list">{tokens.map((token) => <div key={token.id}><span><strong>{token.name}</strong><small>{token.token_prefix}… · {token.expires_at ? `${new Date(token.expires_at).toLocaleDateString("ja-JP")}まで` : "無期限"}</small></span><button className="danger-link" onClick={() => revokeToken(token)}>無効化</button></div>)}{!tokens.length && <p className="muted-text">有効なMCPトークンはありません。</p>}</div></section>{user.role === "admin" ? <><section className="settings-section panel"><div><h2>ユーザー管理</h2><p>管理者のみ、利用者の権限と利用状態を変更できます。</p></div><div className="user-table">{users.map((item) => <div key={item.id}><span className="avatar">{item.name[0]}</span><span><strong>{item.name}{item.id === user.id ? "（自分）" : ""}</strong><small>{item.email}</small></span><select aria-label={`${item.name}の権限`} value={item.role} onChange={(e) => updateManagedUser(item, { role: e.target.value as User["role"] })}><option value="user">一般ユーザー</option><option value="admin">管理者</option></select><label className="switch"><input aria-label={`${item.name}の利用状態`} type="checkbox" checked={item.is_active} disabled={item.id === user.id} onChange={(e) => updateManagedUser(item, { is_active: e.target.checked })}/><span /></label></div>)}</div></section><section className="settings-section panel backup-section"><div><h2>管理者データツール</h2><p>バックアップの作成と復元は管理者だけが実行できます。</p></div><div className="backup-actions"><article><span className="backup-icon">↓</span><div><strong>バックアップを書き出す</strong><p>あなたの家計データをバージョン付きJSONとして保存します。</p></div><button className="secondary" onClick={backup}>ファイルを保存</button></article><article><span className="backup-icon">↑</span><div><strong>バックアップから復元</strong><p>検証後、現在の家計データを置き換えます。</p></div><input ref={fileRef} hidden type="file" accept="application/json,.json" onChange={(e) => restore(e.target.files?.[0])}/><button className="secondary" disabled={restoring} onClick={() => fileRef.current?.click()}>{restoring ? "確認中…" : "ファイルを選択"}</button></article></div></section><section className="settings-section panel danger-zone"><div><h2>家計データを削除</h2><p>すべての口座・カテゴリ・取引・定期取引を削除します。ユーザーアカウントは残ります。</p></div><button className="danger-link" onClick={deleteAll}>すべての家計データを削除</button></section></> : <section className="settings-section panel restricted-section"><span>🔒</span><div><h2>管理者向け機能</h2><p>ユーザー管理、バックアップ、復元、一括削除は管理者だけが利用できます。</p></div></section>}{editing && <Modal title="プロフィールを編集" close={() => setEditing(false)}><form className="modal-form" onSubmit={saveProfile}><label><span>表示名</span><input name="name" required defaultValue={user.name}/></label><label><span>タイムゾーン</span><select name="timezone" defaultValue={user.timezone}><option value="Asia/Tokyo">Asia/Tokyo</option><option value="UTC">UTC</option></select></label><label><span>通貨</span><select name="currency" defaultValue={user.currency}><option value="JPY">日本円（JPY）</option><option value="USD">米ドル（USD）</option><option value="EUR">ユーロ（EUR）</option></select></label><div className="modal-actions"><button type="button" className="secondary" onClick={() => setEditing(false)}>キャンセル</button><button className="primary">保存</button></div></form></Modal>}{passwordOpen && <Modal title="パスワードを変更" close={() => setPasswordOpen(false)}><form className="modal-form" onSubmit={changePassword}><label><span>現在のパスワード</span><input name="current_password" type="password" autoComplete="current-password" required/></label><label><span>新しいパスワード</span><input name="new_password" type="password" minLength={10} autoComplete="new-password" required/></label><label><span>新しいパスワード（確認）</span><input name="confirm_password" type="password" minLength={10} autoComplete="new-password" required/></label><p className="form-help">10文字以上で、英字と数字を含めてください。</p><div className="modal-actions"><button type="button" className="secondary" onClick={() => setPasswordOpen(false)}>キャンセル</button><button className="primary">変更する</button></div></form></Modal>}</div>;
}
